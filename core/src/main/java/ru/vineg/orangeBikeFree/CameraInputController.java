...
public class CameraInputController extends GestureDetector {
    ...
    private float dragSquareSize=6;
	...
	protected CameraInputController(final CameraGestureListener gestureListener, final OrthographicCamera camera) {
	...
	public CameraInputController(final OrthographicCamera camera) {
	...
	public void update() {
		if (rotateRightPressed || rotateLeftPressed || forwardPressed || backwardPressed) {
			...
			if (rotateLeftPressed)
				camera.rotate(camera.up, delta * rotateAngle);
			if (forwardPressed) {
				camera.translate(tmpV1.set(camera.direction).scl(delta * translateUnits * camera.zoom));
				if (forwardTarget)
					target.add(tmpV1);
			}
			if (backwardPressed) {
	...
	protected boolean process(float deltaX, float deltaY, int button) {
		if (button == rotateButton) {
			...
			camera.translate(tmpV1.set(camera.direction).crs(camera.up).nor().scl(-deltaX * translateUnits*camera.zoom));
			camera.translate(tmpV2.set(camera.up).scl(-deltaY * translateUnits*camera.zoom));
			if (translateTarget)
	...
	public boolean touchDragged (int screenX, int screenY, int pointer) {
        if(Math.abs(startX-screenX)+Math.abs(startY-screenY)>dragSquareSize){
            dragging=true;
        }
	...
	public boolean zoom(float amount) {
		...
		if (!alwaysScroll && activateKey != 0 && !activatePressed)
			return false;
        double v = Math.exp(-amount);
        camera.zoom= (float) Math.min(camera.zoom*v,100f);
		if (scrollTarget)
			target.add(tmpV1);
		if (autoUpdate)
			camera.update();
