...
public class LevelRenderer implements Disposable, ILevelRenderer {
    ...
    public void render(SpriteBatch batch, OrthographicCamera camera) {
        ...
        render(batch, camera, layers);
    }
	

	public void render(OrthographicCamera camera, int[] layers, boolean ownBatch) {
    ...
    public void render(OrthographicCamera camera,int[] layers) {
        render(m_batch,camera,layers);
    }

    private void render(SpriteBatch batch, OrthographicCamera camera, int[] layers) {
        ...
        for (int i = 0; i < layers.length; ++i) {
            ...
            for (int j = 0; j < textures.size; ++j) {
                ...
                texture.region.getTexture().setFilter(Texture.TextureFilter.Linear, Texture.TextureFilter.Linear);
                ...
                if (m_cameraRectangle.overlaps(m_box) ||
                    ...
                    float flippedX = Math.signum(texture.scaleX);
                    float flippedY = Math.signum(texture.scaleY);
                    texture.region.flip(flippedX *((texture.region.isFlipX()?-1:1))<0, flippedY *((texture.region.isFlipY()?-1:1))<0);
                    float originX = texture.originX;
                    if(flippedX<0){
                        originX =texture.region.getRegionWidth()- originX;
                    }
                    float originY = texture.originY;
                    if(flippedY<0){
                        originY =texture.region.getRegionHeight()- originY;
                    }
                    Color color = batch.getColor();//get current Color, you can't modify directly
                    float oldAlpha = color.a; //save its alpha


                    color.a = 1f; //ex. scale = 0.5 will make alpha halved
                    batch.setColor(color); //set it
                    batch.draw(texture.region,
                            texture.position.x * m_mpp - originX,
                            texture.position.y * m_mpp - originY,
                            originX,
                            originY,
                            texture.region.getRegionWidth(),
                            texture.region.getRegionHeight(),
                            Math.abs(texture.scaleX * m_mpp),
                            Math.abs(texture.scaleY * m_mpp),
