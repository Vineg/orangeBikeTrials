...
import ru.vineg.gleed.Polyline;
...
public class LevelLoader extends AsynchronousAssetLoader<Level, LevelLoader.GLEEDParameter > implements Disposable {
	...
	static private Logger s_logger = new Logger("LevelLoader", Logger.INFO);
	 ...
	 * @param parameter custom params (not used)
	 */
	@Override
...
//						dependencies.add(new AssetDescriptor(m_pathRoot + "/" + pathParts[pathParts.length - 1], Texture.class));
	...
	private void loadTextureElement(TextureElement texture, Element item) {
		...
		texture.originY = texture.region.getRegionHeight()-
                Float.parseFloat(origin.getChildByName("Y").getText());
		...
		if(Boolean.parseBoolean(item.getChildByName("FlipHorizontally").getText())){
            texture.scaleX*=-1;
            texture.originX=texture.region.getRegionWidth()-texture.originX;
        }
		if(Boolean.parseBoolean(item.getChildByName("FlipVertically").getText())){
            texture.scaleY*=-1;
            texture.originY=texture.region.getRegionHeight()-texture.originY;
        }
	...
	private void loadPathElement(PathElement path, Element item) {
		...
		path.polyline = new Polyline(vertices);
