 ...
 * small stash for problematic keys. Null keys are not allowed. No allocation is done except when growing the table bodySize. <br>
 * <br>
 * This map performs very fast get, containsKey, and remove (typically O(1), worst case O(log(n))). Put may be a bit slower,
 * depending on hash collisions. Load factors greater than 0.91 greatly increase the chances the map will have to rehash to the
 * next higher POT bodySize.
 * @author Nathan Sweet */
public class ObjectIntMap1<K> implements Iterable<ObjectIntMap1.Entry<K>> {
	 ...
	 * backing table. */
	public ObjectIntMap1() {
		this(32, 0.8f);
	}

	/** Creates a new map with a load factor of 0.8. This map will hold initialCapacity * 0.8 items before growing the backing
	 * table. */
	public ObjectIntMap1(int initialCapacity) {
		this(initialCapacity, 0.8f);
	}

	/** Creates a new map with the specified initial capacity and load factor. This map will hold initialCapacity * loadFactor items
	 * before growing the backing table. */
	public ObjectIntMap1(int initialCapacity, float loadFactor) {
	...
	public ObjectIntMap1(ObjectIntMap1<? extends K> map) {
	...
	public void putAll (ObjectIntMap1<K> map) {
	...
	private void putStash (K key, int value) {
		if (stashSize == stashCapacity) {
			// Too many pushes occurred and the stash is full, increase the table bodySize.
			resize(capacity << 1);
			put(key, value);
			return;
		}
...
//		}
	...
	/** Reduces the bodySize of the backing arrays to be the specified capacity or less. If the capacity is already less, nothing is
	...
	/** Clears the map and reduces the bodySize of the backing arrays to be the specified capacity if they are larger. */
	...
	/** Increases the bodySize of the backing array to accommodate the specified number of additional items. Useful before adding many
	...
	static private class MapIterator<K> {
		...
		final ObjectIntMap1<K> map;
		int nextIndex, currentIndex;
		boolean valid = true;

		public MapIterator (ObjectIntMap1<K> map) {
			this.map = map;
			reset();
		}
	...
	static public class Entries<K> extends MapIterator<K> implements Iterable<Entry<K>>, Iterator<Entry<K>> {
		...
		public Entries (ObjectIntMap1<K> map) {
			super(map);
		}
	...
	static public class Values extends MapIterator<Object> {
		public Values (ObjectIntMap1<?> map) {
			super((ObjectIntMap1<Object>)map);
		}
	...
	static public class Keys<K> extends MapIterator<K> implements Iterable<K>, Iterator<K> {
		public Keys (ObjectIntMap1<K> map) {
			super((ObjectIntMap1<K>)map);
		}
