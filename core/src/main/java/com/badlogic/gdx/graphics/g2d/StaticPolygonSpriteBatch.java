...
/** A StaticPolygonSpriteBatch is used to draw 2D polygons that reference a texture (region). The class will batch the drawing commands
 * and optimize them for processing by the GPU.
 * <p>
 * To draw something with a StaticPolygonSpriteBatch one has to first call the {@link StaticPolygonSpriteBatch#begin()} method which will
 * setup appropriate render states. When you are done with drawing you have to call {@link StaticPolygonSpriteBatch#end()} which will
 * actually draw the things you specified.
 * <p>
 * All drawing commands of the StaticPolygonSpriteBatch operate in screen coordinates. The screen coordinate system has an x-axis
 ...
 * <p>
 * A StaticPolygonSpriteBatch is managed. In case the OpenGL context is lost all OpenGL resources a StaticPolygonSpriteBatch uses internally
 * get invalidated. A context is lost when a user switches to another application or receives an incoming call on Android. A
 * SpritStaticPolygonSpriteBatcheBatch will be automatically reloaded after the OpenGL context is restored.
 * <p>
 * A StaticPolygonSpriteBatch is a pretty heavy object so you should only ever have one in your program.
 * <p>
 * A StaticPolygonSpriteBatch works with OpenGL ES 1.x and 2.0. In the case of a 2.0 context it will use its own custom shader to draw
 * all provided sprites. You can set your own custom shader via {@link #setShader(ShaderProgram)}.
 * <p>
 * A StaticPolygonSpriteBatch has to be disposed if it is no longer used.
 * @author mzechner
 * @author Stefan Bachmann
 * @author Nathan Sweet */
public class StaticPolygonSpriteBatch implements Batch {
    private Mesh mesh;
    ...
    /** Constructs a new StaticPolygonSpriteBatch with a size of 2000, the default shader, and one buffer.
     * @see StaticPolygonSpriteBatch#StaticPolygonSpriteBatch(int, ShaderProgram) */
    public StaticPolygonSpriteBatch () {
        this(2000, null);
    }

    /** Constructs a StaticPolygonSpriteBatch with the default shader and one buffer.
     * @see StaticPolygonSpriteBatch#StaticPolygonSpriteBatch(int, ShaderProgram) */
    public StaticPolygonSpriteBatch (int size) {
        this(size, null);
    }

    /** Constructs a new StaticPolygonSpriteBatch. Sets the projection matrix to an orthographic projection with y-axis point upwards,
     ...
     * @param defaultShader The default shader to use. This is not owned by the StaticPolygonSpriteBatch and must be disposed separately. */
    public StaticPolygonSpriteBatch (int size, ShaderProgram defaultShader) {
    ...
    @Override
    public void begin () {
        if (drawing) throw new IllegalStateException("StaticPolygonSpriteBatch.end must be called before begin.");
        renderCalls = 0;
    ...
    @Override
    public void end () {
        if (!drawing) throw new IllegalStateException("StaticPolygonSpriteBatch.begin must be called before end.");
    ...
    public void draw (PolygonRegion region, float x, float y) {
        if (!drawing) throw new IllegalStateException("StaticPolygonSpriteBatch.begin must be called before draw.");
